...
sqrt_one_minus_alphas_cumprod = torch.sqrt(1.0-alphas_cumprod)     # WRITE CODE HERE: Returns square_root(1 - \bar{\alpha}_t)
...
def visualize(data_root, train_batch_size, img_size):
  train_dataloader, _ = get_dataloaders(data_root=data_root, batch_size=train_batch_size, image_size=img_size)
...
def visualize_diffusion(data_root, train_batch_size):
...
def p_sample(model, x, t, t_index):
  ...
  with torch.no_grad():
    ...
    sqrt_one_minus_alphas_cumprod_t = extract(sqrt_one_minus_alphas_cumprod, t,
                                              x.shape)  # WRITE CODE HERE: Same as above, but for sqrt_one_minus_alphas_cumprod
    sqrt_recip_alphas_t = extract(sqrt_recip_alphas, t,
                                  x.shape)  # WRITE CODE HERE: Same as above, but for sqrt_recip_alphas

    p_mean = sqrt_recip_alphas_t * (x - (betas_t / sqrt_one_minus_alphas_cumprod_t) * model(x,
                                                                                            t))  # WRITE CODE HERE: Obtain the mean of the distribution p_\theta(x_{t-1} | x_t)

    if t_index == 0:
      sample = p_mean  # WRITE CODE HERE: Set the sample as the mode
    else:
      posterior_variance_t = extract(posterior_variance, t,
                                     x.shape)  # WRITE CODE HERE: Same as betas_t, but for posterior_variance
      # WRITE CODE HERE
...
def p_sample_loop(model, shape, timesteps):
  ...
  with torch.no_grad():
    ...
    for i in tqdm(reversed(range(0, timesteps)), desc='Sampling', total=T, leave=False):
      img = p_sample(model, img, torch.full((b,), i, device='cuda', dtype=torch.long),
                     i)  # WRITE CODE HERE: Use the p_sample function to denoise from timestep t to timestep t-1
      imgs.append(img.cpu())
...
def p_losses(denoise_model, x_start, t):
  ...
  x_noisy = q_sample(x_start, t,
                     noise)  # WRITE CODE HERE: Obtain the noisy image from the original images x_start, at times t, using the noise noise.
  predicted_noise = denoise_model(x_noisy, t)  # WRITE CODE HERE: Obtain the prediction of the noise using the model.

  loss = F.smooth_l1_loss(predicted_noise,
                          noise)  # WRITE CODE HERE: Compute the huber loss between true noise generated above, and the noise estimate obtained through the model.
...
def t_sample(timesteps, batch_size):
  ...
  ts = torch.randint(0, timesteps, (batch_size,)).long()  # WRITE CODE HERE: Randommly sample a tensor of size (batch_size,) where entries are independently sampled from [0, ..., timesteps-1]

  return ts


def test_sample_loop(model, img, timesteps):
  # Given the model, and the shape of the image, returns a sample from the data distribution by running through the backward diffusion process.
  # Inputs:
  #   model: The denoising model
  #   shape: Shape of the samples; set as (batch_size, 3, 32, 32)
