...
def p_sample(model, x, t, t_index):
    ...
    with torch.no_grad():
        ...
        p_mean = sqrt_recip_alphas_t * (x - (betas_t / sqrt_one_minus_alphas_cumprod_t) * model(x, t))  # WRITE CODE HERE: Obtain the mean of the distribution p_\theta(x_{t-1} | x_t)
...
def p_sample_loop(model, shape, timesteps):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(reversed(range(0, timesteps)), desc='Sampling', total=T, leave=False):
            img = p_sample(model, img, torch.full((b,), i, device=device, dtype=torch.long), i)  # WRITE CODE HERE: Use the p_sample function to denoise from timestep t to timestep t-1
            imgs.append(img.cpu())
